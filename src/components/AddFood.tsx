...
const requiredPositiveNumber = (
  ...
  zod
    ...
    .pipe(
      integer
        ? zod.number().int().positive(positiveMessage)
        : zod.number().positive(positiveMessage),
    );
...
const AddFood: React.FC = () => {
  ...
  const schema = zod.object({
    ...
    quantityAvailable: requiredPositiveNumber(
  ...
  type AddFoodData = formFields & { sellerId: string };

  const resolver = zodResolver(schema) as Resolver<formFields>;
  const addFoodForm = useForm<formFields>({
    defaultValues: {
      name: "",
      description: "",
  ...
  useEffect(() => {
    setValue(
      ...
      {
        shouldValidate: !!touchedFields.foodImages || isSubmitted,
        shouldDirty: true,
        shouldTouch: true,
      },
    );
  ...
  const { mutateAsync: addFoodMutation, isPending: foodIsPending } =
    useMutation({
      mutationFn: addFood,
  ...
  const onSubmit: SubmitHandler<formFields> = (data) => {
    if (isValid) {
      ...
      const payload: AddFoodData = { ...data, sellerId: user.id };
      addFoodMutation(payload);
  ...
  const inputErrorCss = (isError: boolean) =>
    cn(
      "shadow-md py-6 outline-none ring-0 border-[1px] border-gray-300 rounded-full focus-visible:ring-0 focus-visible:outline-none",
      isError && "shadow-red-200",
    );
  ...
  return (
    <div className="my-6 max-w-[60%] mx-auto">
      <Form {...addFoodForm}>
        <form
          onSubmit={addFoodForm.handleSubmit(onSubmit)}
          className="space-y-8"
        >
          <FieldSet>
            <FieldLegend>List Food</FieldLegend>
            <FieldDescription>
              Please add the food you want to sell.
            </FieldDescription>
            <FieldGroup>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <Field>
                          ...
                          <div
                            className={cn(
                              "space-x-2 border-[1px] border-gray-300 p-2  rounded-full shadow-md",
                              !!errors.categoryId && "shadow-red-200",
                            )}
                          >
                            <DropdownMenu>
                              <DropdownMenuTrigger className="focus:outline-none flex items-center justify-between w-full">
                                <span className="pl-2">
                                  {selectedCategoryName ?? "Select an option"}
                                </span>
                      ...
                      render={({ field }) => (
                        <Field>
                          ...
                          <Input
                            ...
                            {...field}
                            className={inputErrorCss(!!errors.name)}
                          />
                          <FieldError>
                            <FormMessage />
                          </FieldError>
                      ...
                      render={({ field }) => (
                        <Field>
                          ...
                          <Input
                            ...
                            {...field}
                            className={inputErrorCss(!!errors.description)}
                          />
                          <FieldError>
                            <FormMessage />
                          </FieldError>
                          ...
                          <TagInput
                            ...
                            styleClasses={{
                              ...
                              inlineTagsContainer: cn(
                                inputErrorCss(!!errors.ingredients),
                                "!py-3",
                              ),
                            }}
                          />
                  ...
                  <div className="flex itmes-center gap-4">
                        ...
                        render={({ field }) => (
                          <Field>
                            ...
                            <Input
                              ...
                              {...field}
                              className={inputErrorCss(!!errors.price)}
                            />
                            <FieldError>
                              <FormMessage />
                            </FieldError>
                    ...
                    <FormItem>
                      <FormField
                        ...
                        render={({ field }) => (
                          <Field>
                            <FieldLabel htmlFor="quantityAvailable">
                              Food Quantity *
                            </FieldLabel>
                            <Input
                                    ...
                                    <CardContent className="p-0">
                                      <div className="group !w-80 !h-80 relative aspect-[16/9] overflow-hidden rounded-lg">
                                        ...
                                        <Button
                                          ...
                                          size="icon"
                                          onClick={() =>
                                            deleteFoodListingImageeMutation(
                                              img.public_id,
                                            )
                                          }
